...
(function loop() {
  ...
  try {
    ...
    // A broken mesh must never permanently freeze the view: drop the
    // offending groups and keep the camera/orbit loop alive.
    console.error('Render error, clearing 3D groups to recover:', err);
    disposeGroup(solidGrp); disposeGroup(diskGrp); disposeGroup(curveGrp); disposeGroup(markerGrp);
    showErr('Error al renderizar la escena, se reinició la visualización: ' + err.message);
  }
})();
...
// Returns raw (already-positioned, color-tagged) geometries for one disk/washer
// slice instead of Mesh objects, so the caller can merge many slices into a
// handful of draw calls (critical once n gets into the dozens).
function buildDiskGeometries(pos, R, r, thick, axis, color, segs) {
  const hollow = r > 0.005;
  const out = { outer: null, inner: null, caps: [], rings: [] };

  const oGeo = new THREE.CylinderGeometry(R, R, thick, segs, 1, hollow);
  if (axis === 'x') { oGeo.rotateZ(Math.PI/2); oGeo.translate(pos,0,0); }
  else              { oGeo.translate(0,pos,0); }
  out.outer = colorize(oGeo, color);

  if (hollow) {
    const iGeo = new THREE.CylinderGeometry(r, r, thick, segs, 1, true);
    if (axis === 'x') { iGeo.rotateZ(Math.PI/2); iGeo.translate(pos,0,0); }
    else              { iGeo.translate(0,pos,0); }
    out.inner = colorize(iGeo, color);

    const half = thick / 2;
    out.caps.push(
      colorize(buildCap(R, r, pos - half, axis, false), color),
      colorize(buildCap(R, r, pos + half, axis, true), color),
    );

    [pos - half, pos + half].forEach(p => {
      const rGeo = new THREE.RingGeometry(r, r + 0.008, segs);
      if (axis === 'x') { rGeo.rotateY(Math.PI/2); rGeo.translate(p,0,0); }
      else              { rGeo.rotateX(-Math.PI/2); rGeo.translate(0,p,0); }
      out.rings.push(colorize(rGeo, 0xf6ad55));
    });
  }
  return out;
}
...
function buildScene() {
  ...
  try {
    ...
    disposeGroup(solidGrp); disposeGroup(diskGrp); disposeGroup(curveGrp);
    showErr('No se pudo construir la visualización 3D: ' + err.message);
  }
}
...
function buildSceneInner() {
  ...
  if (document.getElementById('chkDisks').checked) {
    ...
    const dx = (b - a) / n;
    const segs = n > 50 ? 16 : n > 25 ? 24 : 36; // fewer slices need less tessellation per slice
    const outers = [], inners = [], caps = [], rings = [];
    for (let i = 0; i < n; i++) {
      const t = a + (i + 0.5) * dx;
      const R = Math.max(0.001, interpR(xs,outer_r,t));
      const r = inner_r ? Math.max(0, interpR(xs,inner_r,t)) : 0;
      const g = buildDiskGeometries(t, R, r, dx*0.85, axis, DISK_PALETTE[i%DISK_PALETTE.length], segs);
      outers.push(g.outer);
      if (g.inner) inners.push(g.inner);
      caps.push(...g.caps);
      rings.push(...g.rings);
    }
    const wallMat = new THREE.MeshPhongMaterial({ vertexColors:true, transparent:true, opacity:0.7, shininess:60, side:THREE.DoubleSide });
    const addMerged = (arr, mat) => {
      if (!arr.length) return;
      const geo = mergeGeometries(arr);
      if (geo) diskGrp.add(new THREE.Mesh(geo, mat));
    };
    addMerged(outers, wallMat);
    addMerged(inners, wallMat.clone());
    addMerged(caps,   wallMat.clone());
    addMerged(rings,  new THREE.MeshBasicMaterial({ vertexColors:true, side:THREE.DoubleSide }));
  }
...
function scheduleRebuild() {
  ...
  rebuildQueued = true;
  requestAnimationFrame(() => { rebuildQueued = false; if (S) buildScene(); });
}
...
['chkSolid','chkDisks','chkCurve'].forEach(id => document.getElementById(id).addEventListener('change', scheduleRebuild));
